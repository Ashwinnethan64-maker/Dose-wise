        ...
        <AnimatedPage className="space-y-6 sm:space-y-8">
            ...
            {medications.length === 0 ? (
                <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
                    className="rounded-2xl border p-8 sm:p-12 md:p-14 text-center themed-modal">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    {medications.map((med) => {
                        ...
                        return (
                            <MedicationCard key={med.id} med={med} onEdit={handleEdit} onDelete={handleDeleteRequest} interactions={interactions} />
                        );
                    })}
                </div>
            )}
